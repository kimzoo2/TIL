...
**InnoDB, MyISAM, Memory** 등 있으며 성능 향상을 위해 버퍼풀과 키캐시 기능 등을 갖고 있음<br />
...
각 스레드 그룹 내에는 할당된 사용자 연결로부터 쿼리 실행을 담당하는 하나 이상의 스레드가 존재
...
query worker thread - 할당된 사용자 연결로부터 쿼리 실행을 담당하는 하나 이상의 스레드
