...
## 단기, 중기, 장기 스케줄링
- 단기 스케줄링 : 운영체제가 프로세스에게 CPU를 할당해주는 문제
- 중기 스케줄링 : 운영체제가 프로세스에게 메모리를 뺏어오는 문제
- 장기 스케줄링 : 운영체제가 프로세스에게 메모리를 할당할지 결정하는 문제

### 장기 스케줄링 (job scheduler)
- 시작 프로세스 중에 어떤 것을 ready queue로 보낼지 결정한다.
...
- 빈 공간에 적재하는 방식은 **최초 적합, 최적 적합, 최악 적합** 방식이 존재한다.
  - **최초 적합** : 처음에 접근한 메모리 공간에 적재하는 방식이다. 검색을 최소화할 수 있고 결과적으로 빠른 할당이 가능하다.
  - **최적 적합** : 빈 공간을 **모두 검색**한 후, 적재될 수 있는 공간 중 **가장 작은 공간**에 적재한다.
  - **최악 적합** : 빈 공간을 **모두 검색**한 후, 적재될 수 있는 공간 중 **가장 큰 공간**에 적재한다.
